	...
	describe('actions', () => {
		describe('listIncompleteUploads', () => {
			it('lists all incomplete uploads in a Bucket', () => {
				...
				const listing = Service().actions.listIncompleteUploads.handler.bind(context)({
				...
				return Promise.delay(100)
					...
					.then(() => listing)
					.then((r) => {
						expect(context.client.listIncompleteUploads.mock.calls[0]).toEqual([
							bucketName,
							prefix,
							recursive,
						]);
			...
			it('rejects if the stream encountered an error', () => {
				...
				return Promise.delay(100)
					.then(() => {
						stream.on.mock.calls.find((e) => e[0] === 'error')[1](
