...
module.exports = {
	...
	actions: {
		...
		putObject: {
			handler(ctx) {
				return this.Promise.resolve({ stream: ctx.params, meta: ctx.meta }).then(
					({ stream, meta }) =>
		...
		presignedPutObject: {
			...
			handler(ctx) {
				return this.Promise.resolve(ctx.params).then(({ bucketName, objectName, expires }) => {
					return new this.Promise((resolve, reject) => {
						this.client.presignedPutObject(bucketName, objectName, expires, (error, url) => {
							if (error) {
								reject(error);
							} else {
								resolve(url);
